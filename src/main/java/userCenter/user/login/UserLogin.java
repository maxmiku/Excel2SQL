...
public class UserLogin extends HttpServlet{
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException{
		 ...
		 * @author lexuan
		 * 
		 * @return status:0成功  1数据库连接失败  2解析请求数据失败  3发现sql注入 4找不到用户名 5用户与登录时的不匹配 7服务器内部错误
		 */
		...
		try {
			...
			if(inj.getString("username")==null || inj.getString("password")==null) {
			...
			if(inj.get("token")!=null) {
				...
				String realToken = MD5Util.md5(inj.getString("username")+inj.getString("password")+(String)session.getAttribute("token"));
			...
			if(!inj.getString("username").equals((String) session.getAttribute("uname"))) {
				retj.put("status",5);
				retj.put("errMsg","请求非法,当前用户不正确,请重新输入用户名");
				out.print(retj);
		...
		retj.put("retData", userLogin(inj,retj,req,session));
		
		out.print(retj);
	}
	
	@SuppressWarnings("deprecation")
	private static JSONObject userLogin(JSONObject inj,JSONObject errj,HttpServletRequest req,HttpSession session) {
		...
		data.put("key", "");
		data.put("loginNotic","");
		String uname = inj.getString("username");
		try {
			ResultSet rs=rsql.pullData("uname=\""+inj.getString("username")+"\"", "users");
			int count=0;
			while(rs.next()) {
				count++;
				if(count>1) {
				...
				String inPass = RSAUtil.decrypt(inj.getString("password"), (String)session.getAttribute("sRsaPriK"));
				
				if(inPass.equals(dbPass)) {
					//密码正确
					data.put("loginStatus", RSAUtil.encrypt("0",(String) session.getAttribute("cRsaPubK")));
					
//					String cRsaPubK = inj.getString("cRsaPubKey");
					String aesKey= MaxMikuUtils.getRandomString(64);
					
					session.setAttribute("aesKey", aesKey);
					session.setAttribute("uid", rs.getString("uid"));
					session.setAttribute("ulevel", rs.getString("ulevel"));
					session.setAttribute("ulogin", 1);
					
//					session.setAttribute("uname", inj.getString("username"));
//					session.setAttribute("cRsaPubK", cRsaPubK);
					String cRsaPubK = (String) session.getAttribute("cRsaPubK");
					wsql.updateData("uname=\""+uname+"\"", "users", "ulast_login=\""+MaxMikuUtils.getRemoteIP(req)+"\", upass_wrong_count=0");
					data.put("key", RSAUtil.encrypt(aesKey, cRsaPubK));
					data.put("skipUrl",session.getAttribute("skipUrl"));
					String loginNotic = rs.getString("unotic");
					if(loginNotic!=null) {
						data.put("loginNotic",RSAUtil.encrypt(loginNotic,(String) session.getAttribute("cRsaPubK")));
						wsql.updateData("uname=\""+uname+"\"", "users", "unotic=null");
					}
					BanTools.cleanBanIndex(req);
					
				}else {
					//密码错误
					data.put("loginStatus", RSAUtil.encrypt("1",(String) session.getAttribute("cRsaPubK")));
					BanTools.increaseBanIndex(req, 1);
